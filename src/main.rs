...
fn main() {
    ...
    thread::spawn(move || {
        loop {
            thread::sleep(Duration::from_secs(1)); // simulate a clock signal every second
    ...
    // Remove this loop if you don't have a reset signal
    loop {
        // Replace `reset_signal_received` with your actual reset signal check
        if reset_signal_received() {
            let mut x_lock = x.lock().unwrap();
            let mut p_lock = p.lock().unwrap();
            *x_lock = BigUint::from_str("987654321098765432109876543210987654321098765432109876543210987654321098765432109876543210987654321098765432109876543210987654321098765432109876543210").unwrap(); // replace "..." with the reset value for x
            *p_lock = BigUint::from_str(
                "104899928942039473597645237135751317405745389583683433800060134911610808289117",
            )
            .unwrap(); // replace "..." with the reset value for p
        }
    }
}
